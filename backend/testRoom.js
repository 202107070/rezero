...
async function cleanup() {
  if (createdRoomIds.length > 0) {
    await pool.query(
      `DELETE FROM rooms
       WHERE id IN (${createdRoomIds.map(() => '?').join(', ')})`,
      createdRoomIds,
    );
  }
  ...
  if (createdUserIds.length > 0) {
    await pool.query(
      `DELETE FROM users
       WHERE id IN (${createdUserIds.map(() => '?').join(', ')})`,
      createdUserIds,
    );
  }
...
async function runTest() {
  ...
  assert(unauthorized.response.status === 401, 'JWT 없는 요청이 차단되지 않았습니다.');
  ...
  assert(invalidRoom.response.status === 400, '빈 방 제목이 차단되지 않았습니다.');
  ...
  assert(createResult.data.maxPlayers === 2, '1/1 최대 인원은 2명이어야 합니다.');
  assert(createResult.data.players === '1/2', '방 생성 후 인원 표시가 올바르지 않습니다.');
  ...
  assert(
    listResult.response.status === 200
      && listResult.data.rooms.some((room) => room.id === roomId),
  ...
  assert(duplicateJoin.response.status === 409, '중복 입장이 차단되지 않았습니다.');
  ...
  assert(fullRoomJoin.response.status === 409, '정원 초과 입장이 차단되지 않았습니다.');
  ...
  assert(forbiddenDelete.response.status === 403, '일반 사용자의 방 삭제가 차단되지 않았습니다.');
  ...
  assert(deleteResult.response.status === 200, '새 방장의 방 삭제에 실패했습니다.');
  ...
  assert(
    !closedRoomList.data.rooms.some((room) => room.id === roomId),
    'CLOSED 상태의 방이 방 목록에 표시됩니다.',
  );
  ...
  assert(privateRoom.data.pwd === 'protected', '비밀번호 원문이 응답에 포함되면 안 됩니다.');

  const privateRoomId = privateRoom.data.id;
  createdRoomIds.push(privateRoomId);
  console.log('PASS: 비공개 방 생성과 비밀번호 숨김');

  const wrongPassword = await request(`/api/v1/rooms/${privateRoomId}/join`, {
    method: 'POST',
    token: guest.token,
    body: { password: 'wrong-password' },
  });
  assert(wrongPassword.response.status === 403, '잘못된 비밀번호가 차단되지 않았습니다.');
  console.log('PASS: 잘못된 방 비밀번호 차단');

  const correctPassword = await request(`/api/v1/rooms/${privateRoomId}/join`, {
    method: 'POST',
    token: guest.token,
    body: { password: 'secret123' },
  });
  assert(correctPassword.response.status === 200, '비공개 방 입장에 실패했습니다.');
  ...
  assert(privateDelete.response.status === 200, '비공개 방 삭제에 실패했습니다.');
  ...
  assert(emptyRoom.response.status === 201, '빈 방 종료 테스트용 방 생성에 실패했습니다.');

  const emptyRoomId = emptyRoom.data.id;
  createdRoomIds.push(emptyRoomId);

  const emptyRoomLeave = await request(`/api/v1/rooms/${emptyRoomId}/leave`, {
    method: 'POST',
    token: extra.token,
  });
  assert(emptyRoomLeave.response.status === 200, '마지막 참가자의 퇴장에 실패했습니다.');
  assert(emptyRoomLeave.data.roomClosed, '빈 방이 종료 상태로 처리되지 않았습니다.');
  ...
  assert(missingRoom.response.status === 404, '존재하지 않는 방 요청이 차단되지 않았습니다.');
